...
def start():
    ...
    #
    # (471, 636, 3)
    paintWindow = np.zeros((width * 2, height * 2, 3), np.uint8) + 255
    ...
    while True:
        ...
        for i in range(len(bpoints)):
            for k in range(1, len(bpoints[i])):
                ...
                cv2.line(paintWindow, bpoints[i][k - 1], bpoints[i][k], color, 2)
        ...
        if cv2.waitKey(1) & 0xFF == ord("q"):
            ...
            # max_line_gap = 100
            # cv2.imwrite("gausImage.png",c)
            # h = hough_lines(c, rho, theta, threshold, min_line_len, max_line_gap)
            # h_gray = grayscale(h)
            # cv2.imwrite("trackingImage.png", h)
            # psm_config = '--psm 10'
            # text = pytesseract.image_to_string(c, lang='eng', config=psm_config)
            # print(text)
            # f = open("output.txt", "w")
            # f.write(text)
            # f.close()
            # image = string_image(text)
            cv2.imwrite("paintImage.png", paintWindow)
            # cv2.imwrite("output.png", image)
            # cv2.imshow("Text Image", image)
            # image.show()
            img_path = "paintImage.png"
            pdf_path = "result.pdf"
